...
export default function InvestmentPropertiesModal({ modalData }) {
  ...
  const isOwnershipEnabled = modalData?.key === "investmentPropertyDetails";
  ...
  const initialValues = useMemo(() => {
    ...
    return {
      ...
      investmentProperties: existing.length ? existing : [],
    };
    ...
    const tail = [
      {
        ...
        renderView: ({ value, record }) => (
          <PopupDisplay
            value={value}
            onClick={() => openLoanModal({ record, form })}
          />
        ),
      },
      {
      ...
      },
      {
        ...
        renderView: ({ value, record }) => (
          <PopupDisplay
            value={value}
            onClick={() => openExpenseModal({ record, form })}
          />
        ),
      },
      {
  ...
  const handleFinish = async (values) => {
    ...
    const raw = Array.isArray(values?.investmentProperties)
      ? values.investmentProperties
      : [];
