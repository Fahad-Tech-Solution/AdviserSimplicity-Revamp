...
export default function BusinessTrustTrusteeInnerModal({ modalData }) {
  ...
  const initialValues = useMemo(() => {
    const existing = parentForm?.getFieldValue?.(basePath);
  ...
  const detailRows = useMemo(
    () =>
      buildDirectorRows(Number(innerCount) || 0, directorRows).map((item, index) => ({
        key: `trustee-inner-${rowIndex}-${index}`,
        formPath: ["directorRows", index],
        rowNumber: index + 1,
        ...item,
      })),
    [directorRows, innerCount, rowIndex],
  );
  ...
  const handleConfirmAndExit = () => {
    ...
    parentForm?.setFieldValue?.(basePath, rows);
    setEditing(false);
    closeModal?.();
  };
