...
export default function EstatePlanningDescriptionModal({ modalData }) {
  ...
  const initialValues = useMemo(
    () => ({
      description: modalData?.initialValues?.estatePlanningdescription || "",
    }),
    [modalData],
  );
  ...
  const handleConfirmAndExit = async () => {
    ...
    const updatedRow = {
      ...currentRow,
      estatePlanningdescription: values?.description || "",
    };
      ...
      >
        <Row gutter={[16, 16]}>
          <Col xs={24}>
            ...
            >
              <TextArea rows={6} disabled={!editing} placeholder="Description" />
            </Form.Item>
          </Col>
          <Col xs={24}>
